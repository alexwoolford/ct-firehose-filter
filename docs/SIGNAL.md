...
## Three streams (tier A / B vs research archive)

Tier A and B are **novelty alert types** (both “first-seen”), not “matches vs novel.” Prod runs **tier A only**. The research archive is a separate stream of enqueued MatchEvents — it is **not** tier B.

```text
watchlist match (enqueue)
    ├─→ archive/matches.jsonl     research: every watchlist hit (SAN list may be compacted)
    └─→ novelty
          ├─ tier A  ≥2 low-df brands, first coalition after burn-in → alerts.jsonl + novelty.db
          └─ tier B  first (brand, host) — OFF unless NOVELTY_TIERS includes B
```

| Stream | Meaning | Prod disk |
|---|---|---|
| **Research archive** | Every **enqueued** match — every watchlist hit | `archive/matches.jsonl` — rotate+gzip; prune oldest **sealed** chunks when the **archive directory** hits 50 GiB ([`ARCHIVE.md`](ARCHIVE.md)). Not a host `df` cap. `all_domains` compact at 32 names by default |
| **Alerts (tier A)** | First-seen low-df×low-df coalition (≤5 brands, SAN + degree gates, optional burn-in) | Full alert → `alerts.jsonl` (**20 GiB** prune); keys stay in `novelty.db` |
| **Tier B** | First-seen `(brand, host)`, skip routine labels | **Not emitted / not stored** under default `NOVELTY_TIERS=A` |

“All alerts forever” is false for JSONL payloads: oldest `alerts.jsonl` chunks can be deleted at the 20 GiB budget while `novelty.db` coalition keys remain (so renewals stay quiet). Single-brand matches are **not** “cached tier B”; with the archive on they land in `matches.jsonl`, which is still not a tier B feed.

**Do not union alerts with the archive to “get all events.”** The archive is every watchlist hit. Alerts are a thinner derived view (first-seen low-df×low-df after burn-in). Join alerts → archive on `event.fingerprint` for `all_domains` (may be a 32-name sample; `san_count` is raw). A later product might watch first-seen launch-shaped hosts; that is not a named stream here.

**Two products, one capture.** Inspect does not drop. Alerts emit only **first-seen low-df×low-df** coalitions after a listen-first burn-in (`NOVELTY_CALIBRATE_SECS` default **21600** in prod compose / `NOVELTY_CALIBRATE_EVENTS`). A brand is high-df when its **event count** (`NOVELTY_MAX_BRAND_DF`, default 25) or **partner degree** (`NOVELTY_MAX_PARTNER_DEGREE`, default 25) hits the cap. Mega-apex CT (Amazon, Azure, …) is almost all solo: event-df saturates in seconds; partner-degree may never move. High-df×customer stays in the archive (`mine_hub_customers`). Unlabeled new SaaS can still look like a tier A alert after unmute — accepted; mine the archive later. Optional `NOVELTY_CANDIDATES` JSONL is the learning feed (first-seen coalitions + degrees) — not the hedge-fund pager. This repo ships **no** name lists.
...
4. **Capture alerts only** (`multi_brand_certs`). The archive stays JSONL on the VM. Do not enable tier B.
...
## What alerts actually are (streams honesty)

**There is one product alert file.** Prod `alerts.jsonl` is tier A only (`tier:"A"`). It is **not** split into family vs vendor streams, and lines are **not** labeled with a relation type.

| Thing | Where it lives | Distinguishable? |
|---|---|---|
| **Alerts (tier A)** | `novelty.db` + `alerts.jsonl` | Untagged novel multi-brand coalitions after strip |
| **Glue hubs** | Learned via event-df / partner-degree | Not an alert type — high-fan-out platforms leave alerts; hub-only leaves still archive |
...
Family-looking pairs (Optum×UHC) and scarce vendor-looking pairs (Gilead×Honeywell) **both land in the same `alerts.jsonl`** today. High-df platforms (dealer.com, files.com, Automattic, …) usually do **not** — event-df / packing degree strip them so alerts are not flooded with commodity SaaS co-tenants. That strip is for **diligence SNR**, not a claim that platform edges are worthless.

**Interestingness ≈ improbability, by use-case:** alerts value rare coalitions; platform penetration values hub×customer counts over time (recoverable from archive — see [`ARCHIVE.md`](ARCHIVE.md#platform-hubs-after-glue-strip-penetration-research)). Amazon in a SAN is “Acme uses AWS,” not a deal.

**Hoard vs screen:** keep the research archive (lossy past `ARCHIVE_MAX_TOTAL_BYTES`, default 50 GiB **of the archive directory** — not host `df`; off-box copy if you need older history). Screen alerts with listen-first + live event-df / partner-degree. That asymmetry preserves technographic edges without flooding the diligence product.
...
Shoestring differentiation that still fits: **time-stamped multi-brand / hub×customer edges on a curated ~752k watchlist**, with alerts as the rare diligence slice and the archive as the denser platform-penetration slice. Competing with HG Insights on coverage/verification is not realistic; selling a narrow CT-derived mosaic feed might be.

### Cold start: family vs vendor at first sight

**You cannot reliably classify family vs vendor at runtime on the first observation.** A first-seen alert only means: this coalition key is new after strip. That is a **hypothesis / diligence lead**, not an equity-proven or vendor-proven relationship.
...
1. **Emit unlabeled alerts** — novel multi-brand co-occurrence.
2. **Promote platforms later** — `mine_glue` / `mine_hub_customers` on the archive → human review. Early false alert lines for a new hub are accepted cold-start debt. **Ingest already captured every watchlist hit.** Live event-df / partner-degree catch up without a shipped list.
3. **Confirm family later** — ownership overlay (deferred); not inventable from CT alone on day one.
4. **Scarce vendor** — what stays rare after platform promotions and without an ownership link (Gilead×Honeywell class). A separate vendor stream is deferred until labels exist.
...
Numbers below are from the **raw 15m MatchEvent dump** before in-process novelty shipped. They explain why alert screening was required — not the current prod posture.

| Layer | Result | Intent met? |
|---|---|---|
| Firehose → watchlist match | Works at scale (~47k brands hit in 15m) | Yes |
| Mega-apex / packing hubs | See [live Oracle funnel](#live-oracle-funnel-listen-first-no-lists) — alerts stay quiet after df warm; archive keeps infra | Yes (alert screen, not capture drop) |
| Human-ready diligence trickle | Raw emit still too hot | No (solved later by alerts) |
...
| Alerts (tier A) | **15** (~2.2/hr) |
| Amazon / Zendesk / Azure coalitions | **0** |
| `novelty_high_df_dropped` | 495 |
| amazonaws `events` | ~1.09M with **1** live partner (not a floor of 25) |

Mega-apex is event-df (seconds). Packing hubs are partner-degree. Unlabeled new SaaS can still look like a tier A alert after unmute — accepted; mine the archive later.

**Verdict on the filter:** engineering goal met (needle-shaped *relative to* CT). Raw emit alone is not human-ready; **warm alerts** is the product trickle (~tens/hour).
...
CT firehose --> exact eTLD+1 --> archive every hit
                    |
                    +--> alerts (6h calibrate, then event-df / partner-degree) --> quiet diligence trickle
```
...
| Ready for Oracle production? | **Internal go-live** = `EGRESS=novelty` + size-capped alerts ([`DEPLOY.md`](DEPLOY.md)); **decision-grade** = not yet ([bar](#why-this-signal-matters-pe--corp-dev-diligence)) |
...
**Naming:** offline `rank_signal` Tier A/B/C ≠ novelty product **tier A / B**. Product defaults to tier A only.
...
# Archive: admin/grafana/argocd hostnames (ASM extract — not alerts)
cargo run --release --example mine_admin -- \
  /var/lib/ct-firehose-filter/archive 50
```

**Glue method:** rank brands on multi-keyword certs by distinct co-brand partners × log(events). Promote only **high-fan-out commodity platforms** after posterior evidence (not on first weird pair) — ESP/WAF/DAM/CRS/MFT/CDN/LMS/status-page/API-docs/deals/CMS/HR/ITSM/vertical website/estimate SaaS. Leave corporate families and scarce B2B co-names alone. Do not hard-filter host labels. alert lines stay unlabeled (family vs vendor is not decided at emit time).
**Read the `rank_signal` tiers carefully:** on a *cold* dump, Tier B looks huge because every host is “first seen.” Durable novelty (SQLite on brand-pairs and hosts) is what turns *pair* renewals quiet; tip CT still mints many unique hosts, so **human ops should start with novelty tier A only**.
...
**Product path:** `EGRESS=novelty` runs tier A in-process in the filter binary on Oracle Always Free (SQLite `novelty.db` + rotated `alerts.jsonl`).
...
|---|---|---|
| **Tier A (default)** | First insert of sorted multi-brand coalition, **size ≤5** (`NOVELTY_MAX_COALITION`) | **887** emitted; **230** oversized dropped (DB still records them) |
| Tier B (opt-in `NOVELTY_TIERS=A,B`) | First `(brand, host)`, skipping routine left-labels | Still ~150k events — tip churn, not renewals |
| Fully ignored | Dump-era: all keywords on then-filled suppress/glue | 20,563 |

```bash
# Offline replay (same tier A logic as EGRESS=novelty)
...
**Semantics:** alert JSONL lines are emitted only when `INSERT OR IGNORE` inserts a new row. Coalition renewals (Optum+UHC again) stay silent after the first sighting; a *new* pair still fires. Under default **tier-A-only**, the `hosts` table is **not** written (avoids brand×host growth). Host rows appear only when `NOVELTY_TIERS` includes `B`.

### `alerts.jsonl` shape (`NoveltyAlert`)

Tagged enum envelope — tier A and B do not share null placeholders.
...
| `coalition` | Present on **tier A only** — sorted brands |
| `brand` / `host` / `novel_hosts` | Present on **tier B only** (opt-in `NOVELTY_TIERS`) |
...
**Retention:** Alert **payloads** in `alerts.jsonl` rotate at 256 MiB and prune when live+archives exceed **20 GiB** — you can lose old alert lines while `novelty.db` still remembers the coalition key. Back up JSONL if you need a durable alert history for customers or case studies.
...
With `EGRESS=novelty`, alerts run **in-process** in the filter. The product trickle is a **stateful delta filter**: emit only **first-seen** coalition keys (`INSERT OR IGNORE`). Repeat Optum+UHC → no alert. State lives in SQLite (`coalitions`; `hosts` only when tier B is enabled), with `PRAGMA journal_mode=WAL`.
...
| Tiers | `NOVELTY_TIERS=A` (default; human-scale). tier B opt-in only |
...
| Hosts table | **tier-A-only does not insert hosts** (avoids unbounded brand×host growth); enable `NOVELTY_TIERS` with `B` only when you accept that cost |
...
**Gold extraction** = durable `EGRESS=novelty` on the Oracle VM (SQLite on disk + size-capped alerts → `alerts.jsonl`). Raw MatchEvent dumps alone are not the product feed.
...
### Screened-in tier A (pre size-cap, 1,117 alerts)
...
Mega coalitions (27–37 brands) are almost always **shared-vendor SAN junk**, not relationship signal. Remining “mega-only” brands for new glue found little recurring platform signal. **Fix:** emit alerts only when `coalition_size ≤ 5` (`NOVELTY_MAX_COALITION=5`). After filter: **887** alerts, **0%** size≥6, **230** oversized still remembered in SQLite (no re-alert flood if policy loosens later).

**Mega-SAN packing** (Firebase Hosting etc.): a cert can list **~100 unrelated SANs** while only 2–5 hit the watchlist — that still passes the brand-size cap. **Fix:** plumb raw leaf SAN count on `MatchEvent` and skip the alert when `san_count > NOVELTY_MAX_SANS` (default **32**). Coalition is still recorded in SQLite.

Stratified label sample (100 rows) written to `/tmp/alert-label-sample.jsonl` for human tags: `true_family | shared_vendor | tld_variant | unknown`. Fill ≥100 labels before claiming precision ≥70%.
...
| High-churn singles | Top noisy brands remain as singles — routine infra, **correct to keep out of alerts** |
| Scarce-brand tier B | Deliberately not in default product; real SSO/VPN gold may live here → **v2 rate-limited channel**, not dump-all hosts |
...
**As a mosaic tile:** load alert coalitions into Neo4j (brand nodes, `CO_NAMED_ON_CERT` edges) for demos and exploratory diligence. **Do not** treat a single alert as investable alpha.

| Bar | Status |
|---|---|
| Pipeline works at 752k | GO |
| Human can skim alert pairs | GO (with size cap) |
...
Do **not** equate “~2k alert lines” with commercial alpha. Multi-year full-CT backfill on Always Free is a **non-goal** (CT is multi-TB; no free bulk dump). Validate cheaply:

1. **Stratified sample (habit):** ~20 alert rows/week; tag each `family | platform | scarce_vendor | junk` (use [`audit_alerts`](../examples/audit_alerts.rs) if helpful). Without labels you cannot tell if the trickle is useful.
2. **Platform loop:** mine clear high-fan-out hubs after review (`mine_glue`, `mine_hub_customers`); accept early false alerts as cold-start debt. Capture continues for hub-only leaves. Live df/degree catch up without a shipped list.

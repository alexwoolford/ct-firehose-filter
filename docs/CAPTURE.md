# Capture contract (alerts sqlite vs local JSONL)
...
| `coalitions` / mute `hosts` | no | Mute keys (`U+001F`); `first_seen` is INTEGER Unix (not a captured fact). Mute `hosts` is empty unless tier B is on (it is not) |
| `brand_degree` / `brand_partners` | **no** | Derived; `amazonaws.com` alone would emit tens of millions of `U` events |
| `archive/matches.jsonl` | **no** | ~78 matches/s, ~7M/day — research; rolling 50 GiB prune |
| `alerts.jsonl` | no | Local product copy of alerts; sqlite is the capturable fact |
